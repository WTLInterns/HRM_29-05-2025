...
export default function Attendance() {
  ...
  const checkExistingAttendance = async (fullName, date) => {
    try {
      ...
      const response = await axios.get(`https://admin.managifyhr.com/api/employee/bulk/${encodeURIComponent(fullName)}/${date}`);
  ...
  const handleStatusWithReason = async (status, reason) => {
    ...
    try {
      ...
      let response;
      let existingStatus = '';
      let updateUrl = `https://admin.managifyhr.com/api/employee/${subAdminId}/${encodedEmployeeName}/attendance/update/bulk`;
      let addUrl = `https://admin.managifyhr.com/api/employee/${subAdminId}/${encodedEmployeeName}/attendance/add/bulk`;
    ...
    try {
      ...
      if (newRecords.length > 0) {
        promises.push(
          axios
            .post(
              `https://admin.managifyhr.com/api/employee/${subAdminId}/${encodedEmployeeName}/attendance/add/bulk`,
              newRecords,
              config
            )
            .then(response => ({
      ...
      if (updateRecords.length > 0) {
        promises.push(
          axios
            .put(
              `https://admin.managifyhr.com/api/employee/${subAdminId}/${encodedEmployeeName}/attendance/update/bulk`,
              updateRecords,
              config
            )
            .then(response => ({
