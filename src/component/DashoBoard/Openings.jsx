...
const SITE_MODE_OPTIONS = ["Work from Office", "Home", "Hybrid"];
...
const Openings = () => {
  ...
  <button className={`px-4 py-2 rounded-lg font-semibold ${'bg-blue-600 text-white'}`} onClick={() => setActiveTab("add")}>Add Openings</button>
</div>
      {activeTab === "add" && (
        <>
          {/* If no records at all, show a friendly message above table */}
          {openings.length === 0 && (
            <div className="mb-6 text-center text-lg font-semibold text-red-500 bg-red-50 dark:bg-slate-800 rounded-lg py-4">
              No records. Please add.
            </div>
          )}
...
<div className="flex justify-between items-center mt-4 gap-4">
  ...
  <div className="flex-1 overflow-y-auto overflow-x-auto px-8 pb-8" style={{ WebkitOverflowScrolling: 'touch' }}>
    <form onSubmit={handleAddOrEdit} className="space-y-4 min-w-[350px] md:min-w-0" style={{ minWidth: '350px' }}>
      ...
      {activeTab === "resume" && (
        <div className="text-center py-10 text-gray-400">Check Resume feature coming soon.</div>
      )}
      {/* Toast */}
